...
def topoMain(G, dfRoutes):
    ...
    # STEP 3. CALCULATES TEMPORAL ACCESSIBILITY OF EACH NODE - RUN ON BUDF
    ...
    # STEP n. PRODUCES BOXPLOTS OF NLINES, NSERVICES AND CENTRALITY MEASURES AT EACH VALUE OF NODE DEGREE K
    '''
    nodeCentral = [[node,
                    ...
                    G.nodes[node][const.GNodeAttribs.ccTime.name],
    ...
    dfNodeCentral = pd.DataFrame(nodeCentral, columns=['node', 'totDeg', 'nLines', 'nServices',
                                                       'ccHops', 'ccDist', 'ccTime',
                                                       'cbHops', 'cbDist', 'cbTime', ])
    # dfNodeCentral.to_csv('./outputs/dfNodeCentral.csv',index=False)
    topoNode.makeBoxplot(dfNodeCentral)
    '''
