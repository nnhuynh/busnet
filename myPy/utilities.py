...
#print('lapsed time is %.2g seconds' % duration)

def pairedDifferenceTest(sample1, sample2):
    sampleDiff = [sample1[i] - sample2[i] for i in range(len(sample1))]
    ax = sns.distplot(sampleDiff, kde=True, hist=True, fit=norm)
    plt.show()

# ======================================================================================================================
def calcCentroidOfStops(G):
    # calculates the centroid of stops
    nNodes = len(G.nodes)
    latCentroid = 0
    lngCentroid = 0
    for node in G.nodes:
        lngCentroid += G.nodes[node][const.GNodeAttribs.Lng.name] / nNodes
        latCentroid += G.nodes[node][const.GNodeAttribs.Lat.name] / nNodes

    return latCentroid, lngCentroid
...
def getRange(dist):
    ...
    '''
    '''
    if dist <= 5:                       return 'under 5km'
    elif (dist > 5) and (dist <= 10):   return '5km - 10km'
    elif (dist > 10) and (dist <= 15):  return '10km - 15km'
    elif (dist > 15) and (dist <= 20):  return '15km - 20km'
    elif (dist > 20) and (dist <= 30):  return '20km - 30km'
    elif (dist > 30) and (dist <= 40):  return '30km - 40km'
    else:                               return 'over 40km'
    '''
    if dist <= 10:                      return 'under 10km'
    elif (dist > 10) and (dist <= 20):  return '10km - 20km'
    elif (dist > 20):                   return 'over 20km'
